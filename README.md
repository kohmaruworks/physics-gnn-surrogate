...
**PDE sketch.** Let \(\mathbf{u}\) denote velocity, \(p\) pressure, \(\rho\) density, \(\nu\) kinematic viscosity, \(T\) temperature, \(\alpha\) thermal diffusivity, and \(\kappa\) the coefficient in the auxiliary pressure equation below. A standard incompressible coupled model on \(\Omega \subset \mathbb{R}^2\) reads
...
**支配方程式のスケッチ:** 速度 \(\mathbf{u}\)、圧力 \(p\)、密度 \(\rho\)、動粘性係数 \(\nu\)、温度 \(T\)、熱拡散率 \(\alpha\)、および補助圧力方程式に現れる係数 \(\kappa\) としたとき、\(\Omega \subset \mathbb{R}^2\) 上の非圧縮連成場の典型的な形は以下のようになります。
